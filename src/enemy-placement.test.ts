...
function generateTestLevel(LEVEL_WIDTH: number = 3200, GROUND_Y: number = 400) {
  ...
  while (x < LEVEL_WIDTH) {
    ...
    const willHaveEnemies =
      !isFirstPlatform && !spawnPlatform && Math.random() < 0.2;
...
describe('Enemy Placement Logic', () => {
  ...
  it('should never place enemies on the first platform', () => {
    ...
    const tubesOnFirstPlatform = tubes.filter(
      (tube) =>
        tube.x >= firstPlatform.x &&
        tube.x < firstPlatform.x + firstPlatform.width
    );
  ...
  it('should never place enemies on the spawn platform', () => {
    ...
    const spawnPlatforms = platforms.filter(
      (plat) => plat.x <= 100 && plat.x + plat.width >= 140 // spawn area with player width
    );
    ...
    spawnPlatforms.forEach((platform) => {
      const tubesOnSpawnPlatform = tubes.filter(
        (tube) => tube.x >= platform.x && tube.x < platform.x + platform.width
      );
  ...
  it('should only place tubes on platforms designated for enemies', () => {
    ...
    tubes.forEach((tube) => {
      const hostPlatform = platforms.find(
        (plat) => tube.x >= plat.x && tube.x < plat.x + plat.width
      );
  ...
  it('should ensure enemy platforms are in expected size ranges', () => {
    ...
    const enemyPlatforms = platforms.filter(
      (p) => p.willHaveEnemies && p.width >= 480
    );
    const regularPlatforms = platforms.filter(
      (p) => !p.willHaveEnemies && p.width <= 320
    );
  ...
  it('should place tubes at random positions within enemy platform boundaries', () => {
    ...
    tubes.forEach((tube) => {
      const hostPlatform = platforms.find(
        (plat) => tube.x >= plat.x && tube.x + tube.width <= plat.x + plat.width
      );
      ...
      expect(tube.x + tubeWidth).toBeLessThanOrEqual(
        hostPlatform!.x + hostPlatform!.width - padding
      );
    });
  });
  ...
  it('should place tubes with sufficient spacing from platform edges', () => {
    ...
    tubes.forEach((tube) => {
      const hostPlatform = platforms.find(
        (plat) => tube.x >= plat.x && tube.x + tubeWidth <= plat.x + plat.width
      );
      ...
      const rightDistance =
        hostPlatform!.x + hostPlatform!.width - (tube.x + tubeWidth);
